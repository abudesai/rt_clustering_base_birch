...
def train_and_predict(num_clusters):        
    ...
    # get trained preprocessor, model, and predictions 
    preprocessor, model, predictions = model_trainer.get_trained_model(data, data_schema, hyper_parameters, num_clusters)            
...
def score(test_data, predictions, data_schema): 
    ...
    predicted_clusters = predictions[prediction_col].values
    clusters = set(predicted_clusters)
    
    if len(clusters) == 1: predicted_clusters[0] = 1
    dbi = davies_bouldin_score(scaled_data, predicted_clusters)
...
def save_scoring_outputs(results, dataset_name, chart_data=None):    
    ...
    if chart_data is None: print(df)
